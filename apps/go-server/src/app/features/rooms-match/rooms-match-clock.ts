...
export function createHostedClock(
  ...
  return {
    ...
    revision: 0,
    players: {
      black: createClockPlayer(config),
      white: createClockPlayer(config),
    },
  };
}

export function advanceHostedClock(
  clock: HostedClockSnapshot,
  now: string,
): ClockAdvanceResult {
...
export function completeHostedClockTurn(
  clock: HostedClockSnapshot,
...
export function activateHostedClock(
  clock: HostedClockSnapshot,
...
function createClockPlayer(config: TimeControlSettings) {
  return {
